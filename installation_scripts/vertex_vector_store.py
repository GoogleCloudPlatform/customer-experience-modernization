...
def get_index_resource_name(
    ...
    for r in list(results.indexes):
        if r.display_name == index_display_name:
...
def get_endpoint_info(
    ...
    for i in list(results):
        if i.display_name == endpoint_display_name:
            ...
            deployed_index_id = endpoint_name.split('/')[-1]
            public_endpoint_domain_name = i.public_endpoint_domain_name
    print(endpoint_name, deployed_index_id, public_endpoint_domain_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--index_display_name")
    parser.add_argument("--endpoint_display_name")
    parser.add_argument("--deploy_display_name")
    parser.add_argument("--gcs_url")
    args = parser.parse_args()
    
    index_display_name=args.index_display_name
    index_description=args.index_display_name
    endpoint_display_name=args.endpoint_display_name
    endpoint_description=args.endpoint_display_name
    ...
    # endpoint_display_name = "csm-index-endpoint1"
    # endpoint_description = "CSM Index Endpoint"

    deploy_display_name = args.deploy_display_name
    deploy_id = args.deploy_display_name

    
    metadata = {
        "contentsDeltaUri": gcs_url,
        "config":{
            "dimensions": 1408,
    ...
    (endpoint_name, deployed_index_id, public_endpoint_domain_name) = get_endpoint_info(
        project_id=args.project_id,
        location=args.location,
    ...
    if endpoint_name:
        ...
        print("Creating new Vector Index endpoint")  
    ...
    with open("marketingEnvValue.json", "r") as jsonFile:
        ...
        data[f"{index_display_name.replace('-','_')}_endpoint"] = public_endpoint_domain_name
        data[f"{index_display_name.replace('-','_')}_id"] = deployed_index_id
